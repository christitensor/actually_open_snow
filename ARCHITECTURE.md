...
/lib
  ...
  /lib/derive/
    ...
    snow-level.ts          # FC-10: NWS snowLevel + Open-Meteo freezing_level_height, resolved against resort elevations
...
1. **P0 slice**: `resorts.json` seed data (Northern Utah/SE Idaho, base/mid/summit elevations) → `/api/forecast/[locationId]` via Open-Meteo → one location page rendering a multi-day + hourly forecast, 24h snow estimate, and snow level vs. resort elevation (FC-10) → favorites (local-only first, no auth yet).
2. **P1 core**: snow reports, historical lookback, avalanche forecasts, NWS severe alerts, base MapLibre map with resort pins + piste overlay + webcam pins (MAP-16), multi-source corroborated conditions summary (SNOW-03).
